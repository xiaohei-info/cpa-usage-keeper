...
export interface TurnStateCounters {
  ...
  /** 可加性字段（契约 §3.1）：generic probe + ticket harvest 合并后的主动采集口径，旧 proxy 不返回。 */
  active_attempts?: number | null;
  active_accepted?: number | null;
  active_rejected?: number | null;
  /** 累计统计起点；缺失表示旧 proxy 未上报。 */
  since?: string | null;
  /** 过去滚动 60 分钟内真实 dispatch 次数；缺失表示旧 proxy 未上报。 */
...
function valid(value: unknown, type: string, field = ''): boolean {
  ...
  if (type === 'string') {
    ...
    if (['server_time', 'at', 'issued_at', 'expires_at', 'last_observed_at', 'last_injected_at', 'next_probe_at', 'since'].includes(field)) return value.endsWith('Z') && Number.isFinite(Date.parse(value));
